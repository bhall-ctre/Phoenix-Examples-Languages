...
/**
 * Description:
 * The PositiionClosedLoop_AuxStraightIntegratedSensor example demonstrates the Talon Auxiliary
 * and Remote Features to perform more complex. This example has the robot performing Position 
 * Closed Loop with an auxiliary closed loop on Integrated Sensors difference (Heading)
 ...
 * Talon calculates the distance by taking the sum of both sensors and dividing it by 2.
 * - 2x Falcon 500 Integrated Sensors, one of both side of robot for Auxiliary Closed Loop on Heading
 * Talon calculates the heading by taking the difference between both sensors
 * 
 * This example has two modes of operation, which can be switched between with Button 2.
 * 1.) Arcade Drive
 * 2.) Position Closed Loop wtih Integrated Sensors and Drive Straight with Integrated
 * Sensors difference
 * 
 * Controls:
...
public class Robot extends TimedRobot {
	...
	public void teleopPeriodic() {
		...
		if(!_state){
			...
			/* Configured for Position Closed loop on Integrated Sensors' Sum and Auxiliary PID on Integrated Sensors' Difference */
